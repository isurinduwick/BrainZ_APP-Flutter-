...
import 'birds_ar.dart';
import 'birds_quizz.dart';
import 'birds_pictures.dart';
// ignore: unused_import
import 'package:brainz_app/model/ar_service.dart';
...
class BirdsScreen extends StatelessWidget {
                ...
                children: [
                  ...
                  _buildActionButton(context, "Pictures", () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (_) => const BirdsPicturesScreen()),
                    );
                  }),
                  ...
                  _buildActionButton(context, "AR", () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(builder: (_) => const BirdsARScreen()),
                    );
                  }),
                  const SizedBox(height: 16),
                  _buildActionButton(context, "AR Quick View", () {
                  ...
                  _buildActionButton(context, "Quizz", () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (_) => const BirdsQuizzScreen()),
                    );
                  }),
                  const SizedBox(height: 16),
  ...
  void _showARSelectionDialog(BuildContext context) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text("Select a Bird for AR"),
          content: SizedBox(
            width: double.maxFinite,
            child: ListView(
              shrinkWrap: true,
              children: [
                _buildARSelectionTile(context, "Parrot"),
                _buildARSelectionTile(context, "Eagle"),
                _buildARSelectionTile(context, "Hawk"),
                _buildARSelectionTile(context, "Owl"),
                _buildARSelectionTile(context, "Peacock"),
              ],
            ),
          ),
          actions: [
  ...
  Widget _buildARSelectionTile(BuildContext context, String birdName) {
    return ListTile(
      leading: const Icon(Icons.flutter_dash, color: Color(0xFF3A4A5C)),
      title: Text(birdName),
      onTap: () {
        final animal = AnimalARModel.getAnimalByName(birdName);
        Navigator.of(context).pop();
          ...
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (_) => ARViewWidget(
                animal: animal,
                showFlightControls: true, // Enable flight animation controls
              ),
            ),
          );
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('$birdName AR model coming soon!')),
          );
        }
      },
    );
